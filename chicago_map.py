...
def draw_result(ax, grid, placed, route):
...
def save_geojson(path, route_ll, target_ll, props):
    import json
    feats = [
        {"type": "Feature",
         "properties": {**props, "role": "route"},
         "geometry": {"type": "LineString",
                      "coordinates": [[round(x, 6), round(y, 6)] for x, y in route_ll]}},
        {"type": "Feature",
         "properties": {**props, "role": "target_outline"},
         "geometry": {"type": "LineString",
                      "coordinates": [[round(x, 6), round(y, 6)] for x, y in target_ll]}},
    ]
...
def run_shape(grid, G_proj, edges_gdf, svg, cfg, outdir):
    ...
    ranked = search_placement(contour, grid, {**cfg, "svg_path": svg})
    cost, placed, route = ranked[0]
    if len(route) < 2:
        print(f"  {name}: no route found")
        return None, (name, placed, route)
    ...
    draw_result(ax, grid, placed, route)
    ax.legend(loc="lower right", frameon=True)
    ax.set_title(f"'{name}' running route on real Chicago streets (OSM)\n"
...
def gallery(panels, grid, edges_gdf, path):
    ...
    for ax in axes[n:]:
        ax.set_axis_off()
    for ax, (name, placed, route) in zip(axes, panels):
        draw_streets(ax, edges_gdf)
        draw_result(ax, grid, placed, route)
        ax.set_title(name)
